...
namespace Utilities
{
    namespace Audio
    {
        ...
        { 
            ...
            public void MuteMusic(bool _val)
            {
                tracks[0].Mute = _val;
            }

            public void MuteSFX(bool _val)
            {
                tracks[1].Mute = _val;
                tracks[2].Mute = _val;
            }
            ...
            private bool isInit;
            private Queue<AudioJob> initQueue;
            ...
            public AudioJob WaitForInit
            {
                set
                {
                    if (initQueue == null)
                        initQueue = new Queue<AudioJob>();

                    initQueue.Enqueue(value);
                }
            }
            ...
            private void Update()
            {
                if (!isInit || initQueue == null)
                    return;

                if (initQueue.Count == 0)
                    return;

                var audioAct = initQueue.Dequeue();
                if (audioAct.action == AudioAction.START)
                    PlayAudio(audioAct.type, audioAct.fade, audioAct.delay);
                else if (audioAct.action == AudioAction.STOP)
                    StopAudio(audioAct.type, audioAct.fade, audioAct.delay);
                else if (audioAct.action == AudioAction.RESTART)
                    RestartAudio(audioAct.type, audioAct.fade, audioAct.delay);
            }
            ...
            public void PlayAudio(AudioName type, bool _fade = false, float _delay = 0.0f)
            {
                if (!isInit)
                {
                    WaitForInit = new AudioJob(AudioAction.START, type, _fade, _delay);
                    return;
                }
            ...
            public void StopAudio(AudioName type, bool _fade = false, float _delay = 0.0f)
            {
                if (!isInit)
                {
                    WaitForInit = new AudioJob(AudioAction.STOP, type, _fade, _delay);
                    return;
                }
            ...
            public void RestartAudio(AudioName type, bool _fade = false, float _delay = 0.0f)
            {
                if (!isInit)
                {
                    WaitForInit = new AudioJob(AudioAction.RESTART, type, _fade, _delay);
                    return;
                }
            ...
            public void PlayProvidedAudio(string _trackName, AudioClip _clip)
            {
                foreach (AudioTrack _track in tracks)
                {
                    if (!_track.trackName.Equals(_trackName))
                        continue;

                    if (_track.AdSource != null)
                    {
                        _track.AdSource.clip = _clip;
